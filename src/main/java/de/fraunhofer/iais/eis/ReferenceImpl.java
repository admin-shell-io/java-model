...
public class ReferenceImpl implements Serializable, Reference {
	...
	@JsonIgnore
	java.util.List<TypedLiteral> label = Arrays.asList(new TypedLiteral("Reference", ""));
	//List of all comments of this class
	@JsonIgnore
	java.util.List<TypedLiteral> comment = Arrays.asList(new TypedLiteral("Reference to either a model element of the same or another AAs or to an external entity. A reference is an ordered list of keys, each key referencing an element. The complete list of keys may for example be concatenated to a path that then gives unique access to an element or entity.", "en"));
	...
	*/
	@NotEmpty@JsonAlias({"https://admin-shell.io/aas/3/0/RC01/Reference/key", "referenceKey"})
	 java.util.ArrayList<? extends Key> _referenceKey;
	...
	java.util.ArrayList<? extends Key> getReferenceKey() {
		return _referenceKey;
	}

	final public void setReferenceKey (java.util.ArrayList<? extends Key> _referenceKey_) {
